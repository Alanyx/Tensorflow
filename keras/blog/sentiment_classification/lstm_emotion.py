...
if __name__ == '__main__':
    ...
    def doc2num(s, maxlen):
        s = [i for i in s if i in word_set]  # 去掉低频词，通过集合遍历的方式
        s = s[:maxlen] + [''] * max(0, maxlen - len(s))  # 归一化长度为100，长的截断，短的补全
        return list(abc[s])
    ...
    y = y.reshape((-1, 1))  # 调整标签形状,-1即为缺省，按其他固定下来的维度，总元素数量不变的原则自行填充
    ...
    #  总结：句子转为词组，词组转为由词向量组成的定长"文档"向量，最后对应上标签，喂给lstm
